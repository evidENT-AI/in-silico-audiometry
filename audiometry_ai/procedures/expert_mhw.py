"""
Main audiometry testing implementation using expert audiologist like modified Hughson-Westlake procedure.
"""
...
class ExpertModifiedHughsonWestlakeAudiometry:
    ...
    def _get_starting_level(self, frequency):
        ...
        ## use expert prior to 'judge' starting point (info leak)
        if self.hearing_profile_data[1000] <= 40:
            start_level = DEFAULT_STARTING_LEVEL
        else:
            start_level = DEFAULT_EXPERT_STARTING_LEVEL
        test_level = self._adjust_level(start_level, max_level)
        max_level_count = 0
        ...
        while not self._respond(test_level, frequency):
            ...
            if test_level < 80:
                test_level = self._adjust_level(test_level + 20, max_level) # increase adjustment to speed up test
            else:
    ...
    def _find_threshold(self, frequency):
        ...
        while phase == 'initial':
            ...
            if response:
                ...
                if test_level < 80:
                    test_level = self._adjust_level(test_level + 20, max_level) # Increase step size to 20 to speed up performance
                else:
                    test_level = self._adjust_level(test_level + 5, max_level)
        ...
        while True:
            ...
            if sum(sum(n.values()) for n in n_tests.values()) > EXPERT_MAX_ITERATIONS:
